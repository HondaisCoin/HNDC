UNIX BUILD NOTES
====================
Some notes on how to build HondaisCoin in Unix. 
...
This will build hondaiscoin-qt as well if the dependencies are met.
...
memory available when compiling HondaisCoin Core. With 512MB of memory or less
...
If you want to build HondaisCoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a hondaiscoin-qt executable will be
built by default.

Notes
-----
The release is built with GCC and then "strip hondaiscoind" to strip the debug
...
# Pick some path to install BDB to, here we create a directory within the hondaiscoin directory
...
# Configure HondaisCoin Core to use our own-built instance of BDB
cd $BITCOIN_ROOT
...
cryptography functions, and therefore cannot be used for HondaisCoin (or other
...
Security
--------
To help make your hondaiscoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, hondaiscoin should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, hondaiscoin may be compiled in
