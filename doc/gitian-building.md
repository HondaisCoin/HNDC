...
*Setup instructions for a gitian build of HondaisCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the HondaisCoin
...
- [Building HondaisCoin](#building-hondaiscoin)
...
Clone the git repositories for hondaiscoin and gitian,

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/hondaiscoin/hondaiscoin
```
...
In [doc/release-process.md](release-process.md) in the hondaiscoin repository under 'Fetch and build inputs'.
...
Building HondaisCoin
----------------

To build HondaisCoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the hondaiscoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/hondaiscoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/hondaiscoin/hondaiscoin
...
For example:
```bash
URL=https://github.com/langerhans/hondaiscoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit hondaiscoin=${COMMIT} --url hondaiscoin=${URL} ../hondaiscoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit hondaiscoin=${COMMIT} --url hondaiscoin=${URL} ../hondaiscoin/contrib/gitian-descriptors/gitian-win.yml
```
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/hondaiscoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hondaiscoin-build.assert
```
...
[hondaiscoin/gitian.sigs](https://github.com/hondaiscoin/gitian.sigs/) repository, or
