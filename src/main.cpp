...
// Copyright (c) 2013-2014 The HondaisCoin developers
...
#if defined(NDEBUG)
# error "HondaisCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "HondaisCoin Signed Message:\n";
...
//TODO: this isn't identical to hondaiscoin reference client.
...
// Genesis block
        ...
        // Premine: First block is blocks 2,500,000,000 HNDC
        nSubsidy = 2500000000 * COIN;
    } else if (nHeight < 10000) {
        // Bonus reward for block 2 to 10,000 of 5,000HNDC
        nSubsidy = 5000 * COIN;
    } else if (nHeight < 25000) {
        // Bonus reward for block 10,000 - 25,000 of 10,000HNDC
        nSubsidy = 2000 * COIN;
...
static const int64_t nTargetTimespan = 10 * 30; // HondaisCoin: every 5min
static const int64_t nTargetTimespanNEW = 60 ; // HondaisCoin: every min
static const int64_t nTargetSpacing = 30; // HondaisCoin: 30sec 
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // HondaisCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("hondaiscoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in HondaisCoin
...
bool AcceptBlockHeader(CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // HondaisCoin: reject v2 and v1 blocks at the same time, only check once
        if (block.GetBaseVersion() < 3) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    // HondaisCoin: reject ONLY if block.nVersion=3 has a supermajority because CBlockIndex::IsSuperMajority
